...
function getDiskBoundaries(stellarMass) {
    // Determine the furthest possible point for a planet
    // We use a power of 0.6 to keep the scaling realistic
    const baseMax = 80; 
    const maxRadius = baseMax * Math.pow(stellarMass, 0.6);
    
    // The closest a planet can possibly form (The soot line/sublimation)
    const minRadius = 0.1 * Math.sqrt(stellarMass);
