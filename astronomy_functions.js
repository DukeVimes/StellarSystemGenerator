...
function getPlanetChemistry(surfaceTemp, escapeVelocity, threshold = 0.01) {
    ...
    for (const [name, gas] of Object.entries(GAS_PROPERTIES)) {
        ...
        if (state === "gas") {
            const vTh = Math.sqrt((2 * R * temp) / (gas.molarMass / 1000));
            if (escapeV > (vTh * RETENTION_FACTOR)) {
...
function calculateStellarThermalBoundaries( total_luminosity ) {
    ...
    // We use the square root of luminosity as the scaling factor
    const scale = Math.sqrt(total_luminosity);

    return {
        totalLuminosity: total_luminosity,
        // The "Frost Line" where water ice can form
        iceLine: Number((2.7 * scale).toFixed(3)),
        // The Goldilocks Zone
        habitableZone: {
            inner: Number((0.95 * scale).toFixed(3)),
            outer: Number((1.37 * scale).toFixed(3))
        }
    };
}
